...
from algorithm import AlgorithmDFS
from algorithmDijkstra import AlgorithmDijkstra
...
class ProjectController(app_manager.RyuApp):
    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        alg = AlgorithmDijkstra(self.adjacency, self.bandwidths,self.switches)
        computation_start = time.time()
        paths = alg.get_optimal_paths(src, dst)
        pw = []
        f=open("demo.txt","w")
        f.truncate(0)
        for path in paths:
